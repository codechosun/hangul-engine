...
`lib/` 폴더가 그 엔진이다. A1의 UTF-8 처리기 하나에서 시작해 파일 스캐너·빈도표·샘플러·해시맵 셋·N-그램·백오프·벡터·행렬·신경망 층·텐서·어텐션·트랜스포머·양자화·역전파·옵티마이저·토크나이저·채팅 템플릿까지 장을 거치며 자랐다.

E2에서 선을 긋고 떼어내면 이렇게 된다.

```
dist/hangul-engine/   파일 43개, 181 KB, 외부 의존성 0
